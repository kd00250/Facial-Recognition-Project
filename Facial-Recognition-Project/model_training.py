...
class ModelTrainer:
    ...
    def train(self, num_epochs=30, early_stop_patience=20):
        ...
        for epoch in range(num_epochs):
            ...
            for images, labels in train_bar:
                ...
                # Try calling model with labels (for models like ArcFace) else fallback to single-argument
                try:
                ...
                # If using a per-iteration scheduler (e.g., OneCycleLR), call scheduler.step() here.
                if self.scheduler is not None and hasattr(self.scheduler, "step") and self.scheduler.__class__.__name__ == "OneCycleLR":
                    self.scheduler.step()
            ...
            # If not using OneCycleLR per-iteration, update scheduler once per epoch here
